...
@njit
def NEN_analyse(Q, H, N, D_blade, d_blade, n_blade, NEN_r_array, NEN_beta_array, NEN_delta_array, imp_thk, r_imp_thk, L_f, B_f, fish_type, intake, n_steps=30): 
    # calculate effective length of fish
    L_eff = length_eff(L_f, B_f, fish_type, intake)
    ...
    v_strike_max_array= np.zeros((H_points, Q_points), dtype=np.float32)
    P_th = np.zeros((H_points, Q_points), dtype=np.float32)
    f_MR = np.zeros((H_points, Q_points), dtype=np.float32)
    P_m = np.zeros((H_points, Q_points), dtype=np.float32)
    ...
    for i in range(0, Q_points):
        ...
        for j in range(0, H_points):
            ...
            N_pump_scale = N_scale(N, H, H_temp)
            ...
            for i_r in range(0, n_steps):
                ...
                dP_th = collision_probability(L_eff, v_m, omega, r, n_blade)
                # determine angles at radial position
                r_angle_lookup = r / R_o
                r_angle_idx, _ = find_nearest(NEN_r_array, r_angle_lookup)
                beta_deg = NEN_beta_array[r_angle_idx]
                ...
                r_thk_idx, _ = find_nearest(r_imp_thk, r * 1e3)
                d = imp_thk[r_thk_idx]
                # mortality factor
...
def collision_probability(L_eff, v_m, omega, r, n_blade, wf=0, alpha=0):
    ...
    P_th = np.maximum(0.0, np.minimum(1.0, P_th))
    return P_th
...
def mortality_factor(L_f, t_blade, v_strike, fish_type):
    ...
    f_MR = np.maximum(0.0, np.minimum(1.0, f_MR))
    return f_MR
...
def plot_result(pump_db, result, duty_db, title):
    ...
    duty_mortality = duty_mort(pump_db, duty_db, P_m)
    plt.text(duty_db['Q'], duty_db['H'], f'  {format(duty_mortality, ".2f")}')
    plt.show(block=False)
    
    return fig


def duty_mort(pump_db, duty_db, P_m):
