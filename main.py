...
def NEN_imp_angle_arrays(beta_array, delta_array, r_array, n=100):
    # curve fit the data to get n datapoints
    beta_array, r_array = pump_curves.angle_curve_fit(
        r_series,
        beta_series,
        n=100
    )

    delta_array, r_array = pump_curves.angle_curve_fit(
        r_series,
        delta_series,
        n=100
    )
    r_array = r_array.values.reshape((np.shape(r_array)[0], 1))
    
    return beta_array, delta_array, r_array
